...
class NeuralNetwork(): # neural network class
    ...
    def __init__(self, input, output, q): # initlizing
        x, input_size = np.shape(input) # getting the shape of input
        y, output_size = np.shape(output) # getting shape of output
        
        np.random.seed(1) # using seed to get the same starting weights each time
    ...
    def sigmoid_derivative(self, x): # sigmoid derivaitve
        return x * (1 - x)
    
    def relu(self, x): # relu activation function to get rid of vanishing gradient problem
        return np.maximum(0, x)
    
    def relu_derivative(self, x): # relu derivative
    ...
    def train(self, training_inputs, training_outputs, iterations, learning_rate):
        ...
        for i in range(iterations):
            ...
            # Backpropgation to adjust weights and biases based on the errors calculated
    ...
    def think(self, inputs): # function to test the neural network
